...
import com.advantage.accountsoap.util.TokenizationHelper;
...
//    @Qualifier("paymentPreferencesRepository")
    ...
    private static final Logger logger = Logger.getLogger(PaymentPreferencesService.class);
    ...
    public PaymentPreferencesStatusResponse addMasterCreditMethod(String cardNumber, String expirationDate,
        ...
        // MF Tokenization Start
        String origCardNumber = cardNumber;
        // If the card is valid, tokenize the number before storing
        TokenizationHelper helper = new TokenizationHelper();
        cardNumber = helper.tokenizeCC(cardNumber);
        logger.info("[PROTECT] Tokenizing original CC #: "+origCardNumber);
        logger.info("[PROTECT] Tokenized value: "+cardNumber);
        // MF Tokenization End
...
//        }
    ...
    public List<PaymentPreferencesDto> getPaymentPreferencesByUserId(long userId) {
        ...
        for (PaymentPreferences pref : prefs) {
            ...
            // If the card is valid, tokenize the number before storing
            TokenizationHelper helper = new TokenizationHelper();
            String cardNumber = helper.detokenizeCC(pref.getCardNumber());
            logger.info("[PROTECT] Loaded Tokenized CC # from DB: "+pref.getCardNumber());
            logger.info("[PROTECT] Detokenized CC #: "+cardNumber);
            // MF Tokenization End
